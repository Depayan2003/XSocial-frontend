...
export default function Register() {
  ...
  // keep step just to avoid breaking UI
  const [step] = useState("OTP");

  const [email, setEmail] = useState("");
  const [name, setName] = useState("");
  const [otp, setOtp] = useState(""); // unused but kept
  const [password, setPassword] = useState("");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black">
      <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-sm">
        ...
        {/* VERIFY block kept but unused (safe) */}
        {step === "VERIFY" && (
          <div className="space-y-3">
            <input
              className="w-full bg-gray-800 text-white rounded px-3 py-2"
              placeholder="OTP (unused)"
              value={otp}
              onChange={e => setOtp(e.target.value)}
            />
          </div>
